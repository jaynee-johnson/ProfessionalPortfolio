...
- You can enable **Channel Lock** to switch from an AND type of omnichannel methodology to an OR type of omnichannel methodology. For example:
  ...
  **Important**: An agent with channel lock may not reach their **Total Contact Count**. Instead, they are limited to the channel capacity that they are currently locked to.
